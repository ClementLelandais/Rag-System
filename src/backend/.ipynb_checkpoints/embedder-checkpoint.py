...
class Embedder:
    ...
    def _get_document_embeddings(self, texts: list[str]) -> np.ndarray:
        ...
        for start_idx in range(0, total, self.batch_size):
            ...
            if self.debug:
                ...
                print(
                    f"[DEBUG] Exemple premier texte : {batch_texts[0][:80]}{'...' if len(batch_texts[0]) > 80 else ''}")
            ...
            if self.debug:
                ...
                print(
                    f"[DEBUG] input_ids shape : {inputs['input_ids'].shape}, attention_mask shape : {inputs['attention_mask'].shape}")
    ...
    def get_embeddings(self) -> np.ndarray:
        ...
        if self.debug:
            print(f"[DEBUG] Généré des embeddings pour {len(self.documents)} documents")
    ...
    def generate_in_file(self, path: str):
        ...
        num_batches = (total + self.batch_size - 1) // self.batch_size
        if self.debug:
            print(f"[DEBUG] Sauvegarde stream des embeddings vers '{path}' ({total} docs, {num_batches} batches)")
    ...
    def encode(self, texts: List[str]) -> np.ndarray:
        """Encode multiple texts → retourne embeddings normalisés"""
        if isinstance(texts, str):
            texts = [texts]
        
        # Utilise sentence-transformers direct
        from sentence_transformers import SentenceTransformer
        model = SentenceTransformer(self.model_name)
        
        embeddings = model.encode(
            texts, 
            normalize_embeddings=True,  # ← CRITIQUE
